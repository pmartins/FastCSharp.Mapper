...
FastCSharp.Mapper supports translating Expressions from one object to another in a separate [package](https://www.nuget.org/packages/FastCSharp.Mapper.Extensions.ExpressionMapping/).
...
FastCSharp.Mapper knows `dto.Item` is mapped to `ol.Item.Name` so it substituted it for the expression.
...
FastCSharp.Mapper also supports mapping flattened (TModel or DTO) properties in expressions to their corresponding (TData) navigation properties (when the navigation property has been removed from the view model or DTO) e.g. CourseModel.DepartmentName from the model expression becomes Course.Department in the data expression.
