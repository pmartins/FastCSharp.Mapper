...
FastCSharp.Mapper will scan the designated assemblies for classes inheriting from Profile and add them to the configuration.
...
By default FastCSharp.Mapper recognizes the prefix "Get", if you need to clear the prefix:
...
By default, FastCSharp.Mapper tries to map every public property/field. You can filter out properties/fields with the property/field filters:
...
By default, FastCSharp.Mapper only recognizes public members. It can map to private setters, but will skip internal/private methods and properties if the entire property is private/internal. To instruct FastCSharp.Mapper to recognize members with other visibilities, override the default filters ShouldMapField and/or ShouldMapProperty :
...
Because expression compilation can be a bit resource intensive, FastCSharp.Mapper lazily compiles the type map plans on first map. However, this behavior is not always desirable, so you can tell FastCSharp.Mapper to compile its mappings directly:
